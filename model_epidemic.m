...
function  [Y_real,defGDP,debGDP,labforce1,labforce2,Lev_a,Lev_ab,Lev_al,Lev_af,Lev_ak,defaults,defaults_k,Hdemand,CPI_b,CPI_l,dprob,lock_duration,detect_new,inf_detected,distance,inf_share2,sus_share2,rec_share2,healthcare,serious_share,inf_cum,inf_new,sus_share,inf_share,rec_share,dead_share,gdp_deflator, Investment,EXP, consumption, Un,stock_bonds,wages_t, DC, totK,I,totE,bankruptcy_rate,et] = model_epidemic(seed, T, par)
...
age=zeros(1,W);
rage=rand(W,1);         %randomise agents' age
...
healthcare(:)=0.04*labforce*alpha;         %healthcare spending is 4% of full employment output
...
%%ACCOUNTING
    ...
    %unemployment rate
    %correct measure of unemployment rate if agents can also be inactive!
    ...
    piB=piB+sum(interests)+sum(interests_k) + bond_interest_rate*bonds(t-1)-interest_deposits+dead_assets;                                             %bank profits  
